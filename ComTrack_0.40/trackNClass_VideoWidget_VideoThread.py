...
class TrackNClass_VideoThread(QThread):
    ...
    def initClassifierParams(self):
        self.classifier = None
        self.labels = None
        self.imageType = "Cropped Detections"
        self.imDim = 128
        self.classify = False

    def updateClassifierControls(self, classifierControls):
        if classifierControls.isChecked() is True:
            self.classify = True
        else:
            self.classify = False
        # Loading model and model elements
        if classifierControls.selectedClassifierPath[0] != '':
            self.classifierPath = classifierControls.selectedClassifierPath[0]
            self.classifier = load_model(self.classifierPath)
        else:
            self.classifier = None
        
        if classifierControls.selectedLabelsPath[0] != '':
            self.labelsPath = classifierControls.selectedLabelsPath[0]
            self.lb = pickle.loads(open(self.labelsPath, "rb").read())
            self.labels = self.lb.classes_
            self.header_outputData.extend(self.labels)
            self.classifyingData = pd.DataFrame(columns=self.header_outputData)
        else:
            self.labels = None
        self.imageType = str(classifierControls.imageType.currentText())
        self.imDim = int(classifierControls.imageDims.value())
    ...
    def run(self):
        if self.videoPath[0] != '':
            ...
            while self.threadActive is True:
                ...
                # If frame is found then we process it
                    ...
                    if self.imageProcessingStatus is True:
                        ...
                        # STEP 3: Classify the detected objects
                        # Calculating probabilities of being a specific species (rCNN classifier)
                        if self.classify is True:
                            if (self.classifier is not None) and (self.labels is not None):
                                ROIS = []
                                for detection in outputData_frame:
                                    ...
                                    ymin = detection[23]
                                    ymax = detection[25]
                                    if self.imageType == "Cropped Detections":
                                        roi = frame[ymin:ymax, xmin:xmax]
                                    elif self.imageType == "Contoured Detections":
                                        ...
                                        mask.fill(0)
                                        cv2.drawContours(image=mask, contours=[detection[28]], contourIdx=0, color=(255, 255, 255), thickness=-1)
                                    ...
                                    try:
                                        ...
                                        roi = cv2.resize(src=roi, dsize=(self.imDim, self.imDim), interpolation=cv2.INTER_CUBIC)
                                        roi = img_to_array(roi)
                                        roi = preprocess_input(roi)
                                        ROIS.append(roi)
                                proposals = np.array(ROIS, dtype="float32")
                                # proposals = np.array(ROIS, dtype="uint8")
                                if len(proposals) > 0:
                                    proba = self.classifier.predict(proposals)
                                    i = 0
                                    for detection in outputData_frame:
                                        for species in range(len(self.labels)):
                                            detection.extend([proba[i, species]])
                                        i += 1
                                        self.classifyingData.loc[len(self.classifyingData)] = detection


                    # STEP 4: Adding & Drawing information on image Frame
                    ...
                    # STEP 5: Scaling Image to the GUI display characteristics and send it to GUI to display
                    ...
                    # STEP 7: Update the frame number and end the loop if last frame is reached
                    if (self.framenumber < self.videoTotalFrames):
            ...
            if (self.imageProcessingStatus is True) & (self.outputParams["outputExportData"] is True) & (self.outputParams["outputDataFile"] != "None") & (len(self.classifyingData) > 0):
                self.classifyingData = self.classifyingData.drop(
                    labels=[
                        'Box_TLX',
                        'Box_TLY',
                        'Box_BRX',
                    ...
                    axis=1)
                self.classifyingData['Frame'] = self.classifyingData['Frame'].astype(int)
                self.classifyingData['ID'] = self.classifyingData['ID'].astype(int)
                self.classifyingData['CentroidX'] = self.classifyingData['CentroidX'].astype(int)
                self.classifyingData['CentroidY'] = self.classifyingData['CentroidY'].astype(int)

                if os.path.exists(self.outputParams["outputDataFile"]):
                    os.remove(self.outputParams["outputDataFile"])
                self.classifyingData.to_csv(self.outputParams["outputDataFile"], index = False, sep='\t')
