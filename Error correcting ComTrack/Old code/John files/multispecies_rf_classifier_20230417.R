...
all.files <- list.files("data/rawdata/")

raw.files <- all.files[grep("labels|TC\.", all.files)]
tc.files <- raw.files[grep("TC", raw.files)]

ms_comtrak <- bind_rows(lapply(tc.files, function(x){
  
  # raw data with species probabilities
  c_raw = read_delim(paste0("data/rawdata/", x))
  
  # validated species names
  c_speciesnames = read_delim(paste0("data/rawdata/", gsub("TC", "labels", x))) %>% 
